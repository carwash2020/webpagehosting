...
| `portal/jobs.html` | Job history, warranty overview, check-up reminders, downloadable receipts, and a one-PDF service history of every job (for a home sale or insurance claim) | No |
| `portal/work-orders.html` | Request Work form (title/description/urgency/photos/preferred slot) + two-way messaging on submitted requests; cancel a still-`submitted` request (with an optional reason Triple H is told), or "Need to cancel?" on one already in progress, which asks in the thread | No |
...
| `cancel-work-order` | true | Client-only -- verifies the request belongs to the caller and is still `submitted`, then sets `cancelled` (service role; clients have no UPDATE policy on work orders). The write is conditional on `status=eq.submitted`, so a cancel racing a staff status change updates nothing (409). Since 2026-09-22 (v3) it takes an optional `reason` (trimmed, 500 chars) and posts "I've cancelled this request in the portal." + reason on the request's thread, which fires the existing `on_work_order_message_send_email` trigger -- the internal team is emailed exactly like any client reply. A failed note never fails the cancel; the response says `notified`. Requests past `submitted` aren't cancellable here: the portal's "Need to cancel?" posts a normal client message instead and leaves the status to staff |
