...
8. ~~Deploy the new `send-quote-followup` edge function and run its
   ...
   Independently re-confirmed still `ACTIVE` with matching source and
   active crons directly against the live project later the same day
   (this branch's own audit pass, before either side of this merge
   knew about the other's identical finding).
