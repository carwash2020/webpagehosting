...
- **Bug 1:** 36 public pages called `navigator.serviceWorker.register('/service-worker.js')`. That is the Workspace worker (scope `/`), so every customer's browser precached the tools app (48 URLs, ~3.3MB on disk), and did it again on every `CACHE_NAME` bump (5-22 a day). The inline comment said it was a no-op that cached nothing; the reason given (installability) has been obsolete since Chrome 108 (Android) / 112 (desktop), which dropped the worker requirement for installing from the menu.
