...
## 2026-09-25 (from the visual lane) -- public visitors install the whole Workspace precache

**Fixed the same day (first bullet), see "Public pages no longer install the Workspace worker" below. The second bullet is still open.**
...
- **Root cause:** `storeSession(session, rememberMe)` has three meanings: `true` (localStorage + a fresh 30-day `remember_until`), `false` (sessionStorage, clear localStorage), and `undefined` (keep the current store and `remember_until`, used by the silent refresh). `persistSession()` wrapped it as `storeSession(fields, !!rememberMe)`, so `undefined` became `false`. Settings' `handleVerifyMfaEnrollSettings()` passed `undefined` on purpose, and got the explicit "don't remember" branch.
- **Fix:** `persistSession()` passes `undefined` through and coerces anything else to a boolean. `signIn()`'s own `!!rememberMe` was left alone: sign-in is always an explicit choice. Every login.html caller passes `pendingRememberMe`, which starts `false` and is only ever set from the checkbox's `.checked`.
- **Gotcha:** wrapping a function that treats `undefined` as its own case with `!!x` (or `x || false`, or `Boolean(x)`) silently deletes that case. Check the callee for an `=== undefined` / `!== undefined` branch before coercing an argument you pass through.
- **Test gotcha:** to run a real tool page with the real auth.js in jsdom, swap its `<script src="/tools/auth.js?v=...">` tag for an inline `<script>` holding auth.js (`tests/tools/mfa-settings-remember-me.test.js`, `openPage()`). `window.eval(AUTH_JS)` in `beforeParse` isn't enough: top-level `const`/`let` in eval code stay in the eval's own scope, so page scripts can't see `SUPABASE_URL` or `AUTH_SESSION_KEY`. Only `function` declarations reach `window`. An inline classic script shares the page's global lexical scope, as the real `<script src>` would. Page-level `let`s (like Settings' `mfaPendingFactorId`) still can't be set from outside, so drive the real flow (`handleStartMfaEnrollSettings()` first) instead.
- **Process gotcha (my own slip in this fix):** never `git add -A` while the suite is running. `tests/scripts/check-undefined-vars.test.js` writes a deliberate duplicate `const MIN_LEAD_HOURS = 2;` into the real `portal/jobs.html` (and a stray `const VAPID_PUBLIC_KEY` into `tools/qrcode-lib.js`), runs the checker, then restores the file in `finally`. The first commit on this branch was staged mid-test and carried the duplicate, a SyntaxError that breaks the whole portal Jobs page. Killing that test mid-run also skips its `finally` and leaves the edit behind. After any interrupted run, and before any commit made while tests run, check `git status` for files you didn't touch and restore them from `main`. Stage named files only.

## 2026-09-25 -- Public pages no longer install the Workspace worker; each worker keeps only its own caches

- **Bug 1:** 39 public pages (36, plus three blog posts that landed while this PR was open) called `navigator.serviceWorker.register('/service-worker.js')`. That is the Workspace worker (scope `/`), so every customer's browser precached the tools app (48 URLs, ~3.3MB on disk), and did it again on every `CACHE_NAME` bump (5-22 a day). The inline comment said it was a no-op that cached nothing; the reason given (installability) has been obsolete since Chrome 108 (Android) / 112 (desktop), which dropped the worker requirement for installing from the menu.
- **Fix 1:** public pages no longer register anything. The same inline spot now removes the worker, and every `th-workspace-*` cache, on devices with no sign of Workspace use: none of `th_auth_session` (local or session storage), `th_sync_last`, `th_tracker_jobs`, `th_invoices`, `th_clients`, and no push subscription on the root registration. A tools page re-registers on its next load anyway (`tools-media-sharing.js`).
- **Bug 2 (found while testing Fix 1):** both workers' `activate` deleted every cache except their own `CACHE_NAME`. Cache Storage is per origin, so on a device with both registrations (the owner's phone after opening the portal), every update of either worker wiped the other's offline copy. Measured in Chromium: the Workspace cache went from 48 precached entries to 13 runtime ones after the portal worker installed. Now each deletes only its own prefix.
- **Gotchas:**
  - **Push is tied to the registration.** Unregistering a device's root worker drops its tools push subscription, and re-registering doesn't restore the server-side endpoint. That's why the cleanup checks `pushManager.getSubscription()` and never unregisters a subscribed device. Don't simplify that away.
  - **After `unregister()` the old worker still controls the open page** until it navigates away, and its fetch handler re-creates `th-workspace-vN` on the page's next request. So the cleanup also clears leftover `th-workspace-*` caches on later visits when no root registration is left.
  - **`th_` is not a tools-only prefix.** Public pages write `th_utm_attribution`, `th_upcoming_booking`, `th_lead_request_id` and others, so the check uses named tools keys. `tests/site-wide/public-service-worker.test.js` fails if a public page or `js/` file starts using one of them, or if a key stops being a tools key.
  - **Scenario tests must count entries, not cache names.** A wiped precache looks fine by name, because the controlling worker re-creates the name with a few runtime entries. The cross-wipe scenario only failed on `main` once it checked for 48 entries.
  - **Don't run the suite in the working tree you're updating.** `check-undefined-vars.test.js` temporarily edits `portal/jobs.html`; a `git reset`/merge mid-run made it fail spuriously. Use a separate worktree (`git worktree add --detach`, symlink `node_modules`).
- **Checked, not changed:** `portalUpdateNow()` (`portal/portal-update.js`, the portal's Update now button) deletes every cache on the origin, the Workspace's included. Its comment says that's deliberate, because both workers read with an unscoped `caches.match()`, so scoping it needs that looked at too. Dev Tools' "Clear service worker and caches" clears everything on purpose.
...
- An in-memory PostgREST/GoTrue/Storage stand-in answered the pages.
...
- Dev Tools: all 7 tabs and every panel.
...
- Both service workers: install, precache, no loop, and a deploy that shows one Update card and does one reload.
...
- **#440's card grid bled onto 16 landing pages.**
  ...
  - **Fix:** the row layout moved under `.service-card:has(> .service-icon)`, with the same `:has(> …)` pattern the site banner uses. Chromium measurements now match 3c17596 exactly on the landing pages and #440 exactly on the homepage.
  - styles.css already had a comment warning about this (the `.areas-links` block): "`.service-card/.services-grid` -- that shared component is still used for … the landing pages' own service lists."
- **Dev Tools' full health check said "1 ISSUE" on every run since 2026-09-21.**
  - **Root cause:** the Live Consistency Check still listed `calendar.html`, which became a redirect stub that day. Stubs have no CSP, manifest or PWA meta, by design. check-consistency.js exempted it; the Dev Tools list, which says it's kept in sync with that EXEMPT list, didn't.
  - **Fix:** removed it from the list. The check now reads ALL CLEAR.
  - This bug predates today's redesigns.

**Found in passing, not fixed (latent, pre-existing): a record type the cloud blob has never seen loses its first edits.**
- **Where:** `applySyncData()` skips a key that is absent remotely (`sync.js` ~584), so no merge base is saved for it. The next push's pre-merge then hits "no base entry → remote wins" (`mergeRecordArrays`, ~405). It takes the cloud's older copy of each record wholesale, and pushes that back.
- **Repro:** start from an empty blob, add a job, let it sync, then edit it before any pull. The next push reverts the edit locally and in the cloud.
- **Production today:** safe. All 47 synced keys exist in the live blob (checked read-only).
- **When it bites:** it will fire for the first records of any new synced key a feature adds, as `th_shift_log` was on 09-23, if they're edited before a pull.
- **Likely fix:** after a successful push, save the pushed arrays as the base.
...
- **Harness blob shape matters.** With an empty `workspace_sync.data`, the tools' first sync takes the first-key path above, and edits made soon after "vanish". Seed every synced key (as live has) before blaming a page.
- **Six digits submit on their own.** tools/login.html auto-submits the six-digit code on `input`. A Playwright `fill('123456')` plus a click submits twice.
- **Autofill races `fill()`.** Leaving the invoice/quote client-name field autofills a blank email (`thAutofillClientFields`). A Playwright `fill()` racing that autofill can double the address. A person sees the field already filled; blur and wait before typing.
