...
## 2026-09-23 -- Booking picker round-4 test red from 5:30 to 6 PM Mountain: the test read the wall clock

Reported directly: `tests/portal/booking-picker-round4.test.js`'s "tapping a time hands the page the exact computed slot..." failed with `Cannot read properties of undefined (reading 'click')`, but only in a daily window. It reproduced on the real clock at 17:40 MDT this session.

**Root cause.** `computeSlotsForDate()` (business-hours.js) drops any slot starting less than `MIN_LEAD_HOURS` (2) from `new Date()`, and `createBookingPicker()` opens on the first day with room. For a 120-minute job in 30-minute steps, the last start is 20:00 on a weekday or Saturday (close 22) and 18:00 on a Sunday (close 20). So once now is past 17:30 (Sunday 15:30), today has exactly one slot until 18:00 (Sunday 16:00), and then none, so the picker moves on to tomorrow. The test took `querySelectorAll('#grid .slot-btn')[1]`, which is undefined in that half hour. A product bug it isn't: one slot left is correct.

**Fix (test only).** `pickerWindow()` now evals a pinned `Date` into the window in `beforeParse`, before business-hours.js and booking-flow.js. It's a `class extends Date` whose no-argument constructor and `Date.now()` return Wednesday 2026-09-23 09:00 MDT, the same pattern `next-appointment-banner.test.js` and `portal-visits.test.js` use for vm contexts. Every picker test in the file, the probe windows included, now computes from the same instant. The second-slot test also asserts there are at least 2 slots, so a future hours change fails with a readable message, not a TypeError.

**Gotchas worth keeping:**
- A jsdom window with `runScripts: 'dangerously'` has its own V8 context and its own `Date`. Pinning the test process's clock doesn't reach it: `mock.timers.enable({ apis: ['Date'] })` moved Node's `Date` to 2020 while the window still read the real time (checked this session). Pin inside the window, in `beforeParse`, before the page scripts run.
- To test any file at a chosen wall-clock time without faketime (not installed here): a scratch `--require` preload passed through `NODE_OPTIONS` that wraps `require('jsdom').JSDOM` so every window with scripts gets a ticking `Date` offset to `PIN_CLOCK`, plus the same shim on Node's own `Date`. Offset rather than frozen, so `waitForCondition`-style timeouts that read `Date.now()` still expire. It isn't committed. It's about 40 lines to rewrite.

**Checked elsewhere (same root cause?):**
- **Wrong at first, corrected the same evening:** two more tests tap a later slot: `booking-manage-link-round3.test.js` ("rescheduling updates the remembered visit's time", `.slot-btn[2]`) and `booking-flow-picker-and-confirm.test.js` ("tapping a time never moves the booking by itself", `[1]`). They wait for that many slots, so they passed inside the 17:30 window, and I first logged them as fine. But their visit is 45 minutes, so their windows fall later. The wait times out on weekdays 18:00-19:00 and 18:30-19:00 Mountain (Sundays 16:00-17:00 and 16:30-17:00). My own full run caught round3 at 18:14. A scan of all three files every 30 minutes from 13:15 to 22:45, on a Wednesday and a Sunday, found exactly those windows and nothing else. When the first day is short, both now tap the next open day, then wait as before. **Lesson:** a check at the one failing time only clears that time. Scan the whole day, because each test's visit length sets its own window.
- The full real-clock suite run for this fix ran 17:42-17:49 MDT, inside the weekday window. Only the known `check-links.py` proxy test failed.
- A full-suite scan with the preload pinned to Sunday 15:45 MT also passed, apart from check-links and two `tools/clients-directory.test.js` tests. Those two are an artifact of the preload, not this bug. That file's bare `vm.createContext()` keeps the real clock while the preload moved Node's `Date` to Sunday. They fail at any pinned date other than today (even 09:00) and pass on the real clock. So when this technique shows a failure, check the same file at a normal hour on a different date before blaming the time of day.

**Also fixed in #405, at the owner's request (a different root cause):** `tests/tools/shift-clock-shell.test.js` fails on a UTC runner from 00:00 to about 03:00 UTC (6 to 9 PM Mountain). There, "3 hours ago" is yesterday, so "Start my day" offers no job-clock suggestion and "End my day" reads "Since 9:23 PM Wed, Sep 23". Worse, a failed test never reaches its `w.close()`, so the job clock's `setInterval(updateTime, 1000)` keeps the file's process alive and `node --test` waits on it forever. CI jobs then hang until GitHub's 6-hour limit instead of failing. **Gotcha for any jsdom test that loads a page with timers:** close windows in `afterEach`, not at the end of the test body. The fix: every window in that file runs at a pinned local noon, and the fixtures are times relative to it (`NOW`). An `afterEach` closes every window, passed or failed. Checked on the real clock at 02:13 UTC, inside the window: the old file hung until killed, and the fixed one passes 14/14 in 3 seconds. With a test forced to fail, it now exits in 2 seconds.
