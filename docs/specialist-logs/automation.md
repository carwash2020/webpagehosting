...
**Fix proposed, not merged**: PR #290 adds
...
loop risk. **Not merged** -- CI/CD changes with `contents: write` need
human sign-off per standing rule, and I haven't watched it fire against
a real `pull_request` event yet (only checked the underlying script's
behavior locally).
