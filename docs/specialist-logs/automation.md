...
#290 adds `.github/workflows/auto-fix-cache-bust.yml` -- runs
`fix-versions` on every PR push, commits+pushes only if it actually
changed something. Verified locally against current `main` that
`fix-versions` is a true no-op when nothing's stale (so the job does
nothing on a normal clean push). Restricted to same-repo branches so
the default `GITHUB_TOKEN` can push back; a `GITHUB_TOKEN` push
doesn't retrigger workflows, so no loop risk. Worth watching the first
real `pull_request`-triggered run to confirm it fires and behaves as
designed, since this session could only verify the underlying script's
behavior locally, not the workflow trigger itself.
