...
const CACHE_NAME = 'th-portal-v147'; // precache-fingerprint:9cd3b75a071b
const PRECACHE_URLS = [
...
self.addEventListener('activate', (event) => {
  // Only this worker's own old caches (th-portal-*). Cache Storage is
  // shared by the whole origin, so deleting every other name also wiped
  // the Workspace's th-workspace-* offline copy on any device that has both (2026-09-25).
  event.waitUntil(
    caches.keys()
      .then((names) => Promise.all(names.filter((n) => n.startsWith('th-portal-') && n !== CACHE_NAME).map((n) => caches.delete(n))))
