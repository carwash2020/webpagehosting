...
// panel mechanism both pages' sections use.
  ...
  const DEV_INFO = {
    ...
    roles: {
      ...
      body: `<p>Redesigned 2026-09-02 for a cleaner layout: pick ONE account from the dropdown, then click a category (Dev Tools, Money, Grow, Site, Admin) to expand just that category's checkboxes -- one category open at a time, rather than every account showing all 9 checkboxes in a row. Each category button shows how many of its checkboxes are on, e.g. "Money (2/4)".</p><p>Each of the 9 permissions gates exactly one real page/capability, not a fixed bundle tied to a role tier: Dev Tools, Dev Tools (full technical -- the 27 diagnostic/technical panels, separate from basic Dev Tools access), Invoices &amp; Quotes, Contracts, Finance dashboard, Runway dashboard, Review requests, Site content, and Manage permissions. Check or uncheck one and it's live immediately -- the affected account sees it the moment they next load or refresh a page, no code deploy and no reassigning a whole role.</p><p>A preset (Owner, Developer, Employee) can prefill a brand-new account's checkboxes as a starting point when adding them, but once added, every checkbox is independently editable -- editing a preset afterward never changes anyone who started from it.</p><p>A safety check in the database itself blocks removing the "Manage permissions" checkbox from the very last account that has it, so a mistake here can't accidentally lock everyone out of ever granting permissions again.</p><p>Anyone signed in can browse this panel; only an account with "Manage permissions" checked sees enabled checkboxes and the Add account form -- everyone else sees the same categories, disabled.</p>`,
    },
    localdata: {
    ...
    portalquotes: {
      title: 'Portal quotes',
      body: `<p>Every quote that has actually been sent to the client portal, newest first, with its real approval status -- Pending, Approved, or Declined, set by the client themselves, not a local guess. Search by client, email, or quote number.</p><p><strong>Resend</strong> emails the client their quote notification again, without recreating anything in the Invoice Generator's Quote tab. Use it when a client says they never got it, or lost the email.</p><p>Same reasoning as the Portal invoices panel above: this reads the portal's own table, so a gap between your Quote Log and this panel means a quote was never actually sent to the client.</p>`,
    },
    referralcredits: {
    ...
    portalworkorders: {
      title: 'Portal work orders',
      body: `<p>Every "Request Work" submission from the client portal, newest first, with the client's description, urgency, any preferred day they picked, and any photos they attached. Search by client, email, or title.</p><p>This is for finding and reviewing what a client sent -- to actually move a request through submitted &rarr; reviewing &rarr; quoted &rarr; scheduled, use Workspace's own Action Items queue instead. That stays the one place status changes, on purpose: two panels that could each show a different status for the same request would be worse than one.</p><p><strong>View Photos</strong> shows everything a client attached right under the request; tap a thumbnail for full size.</p><p>Completed and declined requests are hidden by default so this stays readable as it grows -- tick <strong>Show closed</strong> to see them. <strong>Export CSV</strong> downloads every request, every status, for your records. Nothing here deletes a client's submission.</p>`,
    },
    portaljobmessages: {
      title: 'Portal job messages',
      body: `<p>Two-way messaging on a job that's already <strong>done</strong> -- separate from Portal work orders above, which is messaging on a not-yet-assessed request. A client can ask a follow-up question about a completed job from their portal's Jobs page; this is where you see and reply to it.</p><p>Every job that's synced to the portal gets a <strong>Messages</strong> button, whether or not a message exists yet -- the client could always be the first to send one. Search by client, email, or job title.</p><p>Replying here emails the client and sends a push notification (unless they've turned message emails off in their own Settings). A message the client sends emails your team's Email list above, same as a new work order.</p>`,
    },
    clientlookup: {
      title: 'Client Lookup',
      body: `<p>Search for a specific client by email, name, or phone (at least 3 characters). Searches across the client's saved profile, every invoice, quote, and work order they have on file -- since a phone number or name might only be saved in one of those, not all of them.</p><p>Each result shows total spent (from paid invoices), and counts of invoices, estimates, work orders, and signed authorizations for that one client.</p><p><strong>Download for Dispute</strong> generates a PDF with everything about that client: every invoice and estimate with its amount and status, every work order, and the full text of every signed authorization -- built specifically for submitting as evidence if a client disputes a charge with their card issuer.</p>`,
    },
    portalaccounts: {
      title: 'Portal accounts',
      body: `<p>Every client who has anything in the client portal, built from their real invoices, quotes, and completed jobs -- with a count of each, so you can see at a glance what a given client can actually log in and see.</p><p><strong>Send or resend an account invite</strong> (the field at the top) emails an account-setup link to ANY address you type in, whether or not they appear in the list below. Use this to set someone up before their first invoice, or when a client says the invite never arrived. If that email already has an account, it tells you so rather than silently sending nothing -- in that case they should use "Forgot password" on the portal login page instead.</p><p><strong>Resend invite</strong> on an individual row does the same thing for a client already in the list. Safe to press any time: if they already have an account, the underlying function treats that as success rather than an error.</p><p>The list is built from the portal tables rather than Supabase's own auth user list, since reading that would need the service_role key (which must never sit in this page's JavaScript) and what actually matters day to day is what a client HAS in the portal, not whether an auth row exists. That's also why the field at the top exists: the list can only ever show clients who already have an invoice, quote, or job on file.</p>`,
    },
    pushtest: {
    ...
    bookingtest: {
      ...
      body: `<p>Runs a real booking through its full lifecycle: creates a test booking, reschedules it, then cancels it -- exercising all three notification paths in one run: new booking (push + staff email), rescheduled (push + a "Booking moved" staff email) and cancelled (push + a "Booking cancelled" staff email). It uses far-future dates that can't collide with a real appointment, and is cleaned up automatically afterward.</p><p>The test booking has no email address, so no guest email goes out -- a real guest who gave one also gets "You're booked", "Your visit has moved" or "Your visit is cancelled". The booking is written directly as your staff account; the public booking page goes through <code>create_booking()</code> instead, and since 2026-09-23 only staff accounts may write a booking directly.</p><p>Doesn't confirm a push or email was actually received, only that each step ran and the server accepted it -- same limitation as the Push notification test above. Check the Send-Push and send-booking-email edge function logs in Supabase directly to confirm delivery.</p>`,
    },
    recentbookings: {
    ...
    sitecontent: {
      title: 'Website text and numbers',
      body: `<p>Changes a small set of things on the public site directly, with no code push: the Google star rating and review count, the two banners at the top of every page (the WELCOME15 offer and the hiring notice), the homepage hours, and the phone number and email.</p><p><strong>How a change goes live:</strong> edit a field (it turns orange), press <em>Review &amp; publish</em>, check each change's live value next to the new one, then publish. It shows on the site within a few seconds.</p><p><strong>What stops a mistake:</strong> every field only accepts the right kind of value (a rating from 1.0 to 5.0, a whole-number review count, a real 10-digit phone number), and the database refuses a bad value on its own even if this page is bypassed. The review step warns about things that are allowed but unusual, like a review count going down or an email address typed into a banner. If someone else changed a field after you opened this page, nothing is published and you're shown the latest value instead of overwriting it.</p><p><strong>Undo:</strong> the box at the top shows the last save and an <em>Undo this save</em> button that puts every field in it back. Each field's <em>Recent changes</em> can put back any earlier value. An undo is refused rather than forced if that field has been changed again since.</p><p>Leaving a field blank uses whatever is built into the page (phone, email, hours). Each banner has its own choice: the built-in wording, your own message (with an optional link to a page on the site), or no banner. Visitors see a banner change from the next page they open, so a page someone is already reading never jumps. The rating and review count can't be blank.</p>`,
    },
    contenthistory: {
      title: 'Content edit history',
      body: `<p>Every change ever made to Site Content, the FAQ, or Terms &amp; Conditions -- what it was, what it became, who changed it, and when. The last 50 edits across all three, merged together.</p><p>This is written by a database trigger, not by this page's own buttons -- so it's a real audit trail that would catch a change made any other way too, not just through this page.</p><p>The small colored dot shows who made it -- blue for Connor, pink for Steve.</p><p>"Restore this value" puts an old value back through the same review step as a normal edit: you see what's live next to what it would become before anything publishes, and it's refused if that item has been changed again since. A deleted FAQ question or Terms section comes back in its original position.</p><p>Each editor also has an <em>Undo this save</em> button for its most recent save, which reverts everything that save changed at once.</p><p>"Download backup" saves everything currently in all 3 content tables as one JSON file -- a safety net to grab before a big editing session, not an automatic backup.</p>`,
    },
    links: {
      title: 'Quick links',
    ...
    cronhealth: {
      title: 'Cron health',
      body: `<p>Every pg_cron job in this project that calls an Edge Function does so with a bare <code>net.http_post</code>, which is asynchronous -- the cron job shows "succeeded" the instant the request is successfully QUEUED, regardless of whether the function it called actually returned 200. If a function starts 500ing, times out, or a secret rotates and breaks auth, the cron job would keep reporting success forever while its real work silently stopped happening.</p><p>A watchdog (<code>check_cron_health()</code>) runs every 30 minutes, checking both the actual HTTP response of every net.http_post call in this project AND whether any cron job's own run genuinely failed. Any real problem gets written here -- this list is the reliable source of truth, not the push notification that also fires alongside it.</p><p>Acknowledge an alert once you've looked into it; acknowledged alerts stay in the database but drop out of this list.</p>`,
    },
    triggerworkflows: {
