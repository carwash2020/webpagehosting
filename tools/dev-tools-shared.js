...
// panel mechanism both pages' sections use.
  ...
  const DEV_INFO = {
    ...
    roles: {
      ...
      body: `<p>Each of 9 permissions is its own checkbox per account, not a fixed bundle tied to a role tier: Dev Tools, Dev Tools (full technical -- the 27 diagnostic/technical panels, separate from basic Dev Tools access), Invoices &amp; Quotes, Contracts, Finance dashboard, Runway dashboard, Review requests, Site content, and Manage permissions. Check or uncheck one and it's live immediately -- the affected account sees it the moment they next load or refresh a page, no code deploy and no reassigning a whole role.</p><p>Expanded 2026-09-02 from an original 4: Invoices/Quotes/Contracts/Finance/Runway/Reviews used to be one shared checkbox, and the 27 technical panels used to require Manage permissions specifically -- both split apart so a "Review requests only" account, or an account that can see diagnostics without being able to grant/revoke everyone else's access, are both genuinely possible now.</p><p>A preset (Owner, Developer, Employee) can prefill a brand-new account's checkboxes as a starting point when adding them, but once added, every checkbox is independently editable -- editing a preset afterward never changes anyone who started from it.</p><p>A safety check in the database itself blocks removing the "Manage permissions" checkbox from the very last account that has it, so a mistake here can't accidentally lock everyone out of ever granting permissions again.</p><p>Anyone signed in can see this list; only an account with "Manage permissions" checked sees enabled checkboxes and the Add account form -- everyone else sees the same list, disabled.</p>`,
    },
    localdata: {
