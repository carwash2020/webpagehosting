// Tests for send-quote-followup-index.ts's auth check (2026-09-16
...
// quote-followup emails at pending clients on demand. The only real
// caller is the send-quote-followup-daily cron (see
...
const CRON = fs.readFileSync(repo('sql', 'infra', 'add_quote_followup_email_cron.sql'), 'utf8');

test('send-quote-followup-index.ts requires the bearer token to match SERVICE_ROLE_KEY before doing anything else', () => {
  ...
  const quotesReadIdx = handlerMatch[0].indexOf('await fetchPendingQuotes()');
  assert.ok(authCheckIdx !== -1, 'expected an explicit SERVICE_ROLE_KEY comparison');
  assert.ok(quotesReadIdx !== -1, 'expected quotes to still be read after the check');
  assert.ok(authCheckIdx < quotesReadIdx, 'the auth check must happen before any work is done');
  assert.match(handlerMatch[0], /status: 401/);
});

test('the only real caller (the quote-followup cron) already authenticates with the service_role key this check now requires', () => {
