...
const PROMO_JS = fs.readFileSync(repo('js/promo-banner.js'), 'utf8');
...
for (const page of PAGES_WITH_BANNER) {
  test(`${page} loads promo-banner.js`, () => {
    ...
    // Synchronous since 2026-09-23 (no defer) -- see site-banner-no-layout-shift.test.js.
    assert.match(html, /<script src="\/js\/promo-banner\.js\?v=[a-f0-9]{10}"><\/script>/);
...
test('scripts/check-consistency.js tracks promo-banner.js as a global shared file (cache-bust freshness across root/tools/portal/blog)', () => {
  ...
  assert.match(script, /GLOBAL_SHARED_FILES\s*=\s*\[[^\]]*'js\/promo-banner\.js'/);
});
