...
test('F06/F07: all 7 former .tool-header pages (calendar.html and pos.html retired to redirect stubs 2026-09-21) now use .hub-header, which names the page directly in the sticky bar', () => {
...
test('F15: only two content-width tokens exist now, and every non-workspace desktop rule uses one of them', () => {
  ...
  const narrowPages = ['review-request.html', 'route-planner.html', 'job-detail.html', 'client-detail.html', 'settings.html'];
...
test('F16/F17: one shared base padding rule replaces the old 90px/44px/absent-entirely split', () => {
  ...
  // finance/clients/parts-reference (and pos, until it became a tab on
  // invoice-generator 2026-09-21) had NO base body rule at all
  ...
  for (const page of ['finance.html', 'clients.html', 'parts-reference.html']) {
    const src = fs.readFileSync(path.join(TOOLS_DIR, page), 'utf8');
