...
test('dashboard.html: the payment modal traps focus and closes on Escape, with every call site routed through the shared open/close functions', () => {
  ...
  assert.match(DASHBOARD, /async function startPayment\(invoiceId, signerName, signatureImage\) \{\s*\n\s*openPaymentModal\(\);/);
  assert.match(DASHBOARD, /async function startBulkPayment\(invoiceIds, signerName, signatureImage\) \{\s*\n\s*openPaymentModal\(\);/);
