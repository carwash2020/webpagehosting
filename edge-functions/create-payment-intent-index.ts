...
async function recordCardAuthorization(clientEmail: string, signerName: string, authorizationText: string, amount: number, invoiceId: number, signatureImage: string | null) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { invoice_id, signer_name, signature_image } = await req.json();
    ...
    if (!alreadyHasSavedCard) {
      ...
      await recordCardAuthorization(claims.email, signer_name.trim(), authorizationText, amountCents / 100, invoice.id, signature_image);
    }
