...
// Updated 2026-08-31: backup-business-data.yml was retired the same
// day this project's sensitive backups moved to a private repo,
// replaced by backup-sensitive-data.yml -- this allowlist was missed
// at the time, which would have made Dev Tools' "Backup sensitive
// data" button (already fixed to send the new name) fail here with
// "Unknown or disallowed workflow" regardless, since the old name was
// still the only one recognized.
const ALLOWED_WORKFLOWS = new Set([
  "backup-cms-content.yml",
  "backup-sensitive-data.yml",
  "check-links.yml",
  "lighthouse.yml",
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!(await callerPassesInternalMfa(claims, "trigger-workflow"))) {
